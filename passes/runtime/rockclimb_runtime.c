...
void __rockclimb_recover(void) {
    ...
    /* Restore SP first */
    __asm__ volatile("mov.w %0, SP\n" : : "m"(__nvm_sp));

    /* Restore registers R4-R15 */
    __asm__ volatile("mov.w %0, R4\n"
                     "mov.w %1, R5\n"
                     "mov.w %2, R6\n"
                     "mov.w %3, R7\n"
                     :
                     : "m"(__nvm_regs[0]), "m"(__nvm_regs[1]), "m"(__nvm_regs[2]),
                       "m"(__nvm_regs[3]));

    __asm__ volatile("mov.w %0, R8\n"
                     "mov.w %1, R9\n"
                     "mov.w %2, R10\n"
                     "mov.w %3, R11\n"
                     :
                     : "m"(__nvm_regs[4]), "m"(__nvm_regs[5]), "m"(__nvm_regs[6]),
                       "m"(__nvm_regs[7]));

    __asm__ volatile("mov.w %0, R12\n"
                     "mov.w %1, R13\n"
                     "mov.w %2, R14\n"
                     "mov.w %3, R15\n"
                     :
                     : "m"(__nvm_regs[8]), "m"(__nvm_regs[9]), "m"(__nvm_regs[10]),
                       "m"(__nvm_regs[11]));

    /* Jump to saved PC */
    __asm__ volatile("mov.w %0, PC\n" : : "m"(__nvm_pc));
