...
struct RockClimbContext : public BaseContext {
    ...
    RockClimbContext(std::unique_ptr<EnergyEstimator> est, std::unique_ptr<CFGAnalysis> cfgAnalysis,
                     llvm::LoopInfo *li, const RockClimbParams &p)
        : BaseContext(std::move(est), std::move(cfgAnalysis), li), params(p),
...
inline RockClimbContextResult createRockClimbContext(llvm::Function &F, llvm::LoopInfo &LI,
    ...
    return RockClimbContextResult::ok(std::make_unique<RockClimbContext>(
        std::move(base.estimator), std::move(base.cfg), base.loopInfo, params));
}
