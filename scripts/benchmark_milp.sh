...
# CSV header
HEADER="benchmark,capacitor,basic_blocks,edges,global_variables,milp_variables,milp_constraints,optimal_solution,regions,region_boundaries_inserted,distributed_checkpoints_inserted,__region_prologue,__region_epilogue,__checkpoint_store_reg,__checkpoint_store_mem,__restore_reg,__restore_mem"
...
for bench_path in "${BENCHMARKS[@]}"; do
    ...
    for cap_entry in "${CAPACITOR_CONFIGS[@]}"; do
        ...
        echo "$row_name,$cap_label,$basic_blocks,$edges,$global_vars,$milp_vars,$milp_constrs,$optimal,$regions,$boundaries,$dist_ckpts,$prologue,$epilogue,$store_reg,$store_mem,$restore_reg,$restore_mem" >> "$OUTPUT_CSV"
