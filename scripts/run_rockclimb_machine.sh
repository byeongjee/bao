...
# benchmarks and capacitor sizes. Compiles via compile_rockclimb_machine.sh,
# flashes to MSP430 via mspdebug, and reads runtime counters over UART.
#
# Outputs a CSV summary.
#
# Usage:
#   ./scripts/run_rockclimb_machine.sh [-o output.csv] [-v|--verbose] [bench1 bench2 ...]
#
# If benchmark names are given, only those are run (matched by filename without .c).
# Example: ./scripts/run_rockclimb_machine.sh crc chacha20 rsa
#

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"

OUTPUT_CSV="$PROJECT_DIR/benchmarks/rockclimb_machine_benchmark_summary.csv"
VERBOSE=0
FILTER_BENCHMARKS=()
