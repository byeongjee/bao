...
static const uint8_t prv_a[ECC_PRV_KEY_SIZE] = {
    0x2F, 0x46, 0x53, 0xB8, 0x26, 0x67, 0x5E, 0x49,
    0x72, 0x6B, 0x53, 0xE4, 0x99, 0x55, 0x09, 0x99,
    0xBE, 0x39, 0x47, 0x60, 0xF2, 0x91, 0x87, 0x1F,
    0x8F, 0x69, 0x8F, 0x5A, 0x1D, 0x00, 0x00, 0x00
};
static const uint8_t prv_b[ECC_PRV_KEY_SIZE] = {
    0x3E, 0xC0, 0x0C, 0xAF, 0x89, 0x04, 0x52, 0x39,
    0x97, 0x7D, 0x51, 0x4F, 0xDA, 0x58, 0xAE, 0x16,
    0xA9, 0x63, 0x74, 0x67, 0x94, 0x93, 0xB1, 0xF7,
    0x66, 0x7B, 0x29, 0x1C, 0xCE, 0x00, 0x00, 0x00
};
static const uint8_t pub_a[ECC_PUB_KEY_SIZE] = {
    0xAF, 0x08, 0xF4, 0x95, 0x0A, 0x27, 0x5F, 0x50,
    0x96, 0x38, 0x69, 0x45, 0x9D, 0x40, 0xDF, 0x2B,
    0x25, 0xA9, 0x63, 0x35, 0xD4, 0xAC, 0x61, 0x69,
    0x2D, 0x3D, 0x1F, 0xE2, 0x38, 0x00, 0x00, 0x00,
    0x31, 0xAE, 0x94, 0xD6, 0x9C, 0x7D, 0xA4, 0x44,
    0x67, 0xC7, 0x1E, 0x66, 0xD6, 0x36, 0x92, 0x6F,
    0x8F, 0x42, 0xB9, 0xBF, 0x71, 0xB4, 0x4F, 0x5B,
    0x41, 0xAD, 0xAA, 0x86, 0x82, 0x01, 0x00, 0x00
};
static const uint8_t pub_b[ECC_PUB_KEY_SIZE] = {
    0x70, 0xF7, 0x8E, 0xB9, 0x4E, 0x59, 0x98, 0xF8,
    0xB3, 0x3A, 0x48, 0xE2, 0x3C, 0x12, 0x8A, 0x9C,
    0x06, 0xE9, 0x34, 0xD6, 0xBC, 0x32, 0xAD, 0xDC,
    0xFD, 0x2D, 0x03, 0xA3, 0xC7, 0x00, 0x00, 0x00,
    0x77, 0xAF, 0xA1, 0xA4, 0x31, 0x68, 0xBA, 0xF6,
    0x78, 0x29, 0xCB, 0xA6, 0xDC, 0xA3, 0x52, 0x63,
    0xC0, 0xAB, 0x59, 0x53, 0x0C, 0x74, 0xEC, 0x46,
...
FORCE_INLINE void gf2point_copy(gf2elem_t x1, gf2elem_t y1,
                                const gf2elem_t x2, const gf2elem_t y2)
{
    bitvec_copy(x1, x2);
    bitvec_copy(y1, y2);
}
...
FORCE_INLINE void gf2point_add(gf2elem_t x1, gf2elem_t y1,
                               const gf2elem_t x2, const gf2elem_t y2)
{
...
FORCE_INLINE int ecdh_shared_secret(const uint8_t *private_key,
                                    const uint8_t *others_pub,
                                    uint8_t *output)
{
        ...
        &&  gf2point_on_curve((uint32_t*)others_pub, (uint32_t*)(others_pub + BITVEC_NBYTES)))
    {
        ...
        gf2point_mul((uint32_t*)output,
                     (uint32_t*)(output + BITVEC_NBYTES),
