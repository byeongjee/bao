...
static const uint8_t test_data[INPUT_SIZE] = {
    0x43, 0x68, 0x61, 0x43, 0x68, 0x61, 0x32, 0x30,
    0x20, 0x74, 0x65, 0x73, 0x74, 0x20, 0x64, 0x61,
    0x74, 0x61, 0x20, 0x66, 0x6f, 0x72, 0x20, 0x69,
    0x6e, 0x74, 0x65, 0x72, 0x6d, 0x69, 0x74, 0x74,
    0x65, 0x6e, 0x74, 0x20, 0x63, 0x6f, 0x6d, 0x70,
    0x75, 0x74, 0x69, 0x6e, 0x67, 0x20, 0x62, 0x65,
...
FORCE_INLINE void ChaCha20_block_next(const uint32_t in[16],
                                       uint32_t out[16],
                                       uint8_t **keystream) {
    int i;
    for (i = 0; i < 16; i++) {
...
FORCE_INLINE void ChaCha20_init(ChaCha20_Ctx *c, const key256_t key,
                                 const nonce96_t nonce, uint32_t count) {
...
FORCE_INLINE void ChaCha20_xor(ChaCha20_Ctx *c, const uint8_t *input_buffer,
                                uint8_t *output_buffer, const uint32_t bufflen) {
    uint32_t tmp[16];
