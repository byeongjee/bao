...
- **Otii Ace Pro** — replays the voltage trace on its main output, drives the
  switchboard, and watches the benchmark GPIO pulses on GPI1.
...
- **Schottky diode** — in series between the Otii main output and the board
  ...
  and readback.
- **Saleae Logic** — permanently wired to P3.4 as in
  [saleae.md](saleae.md); used for verification and debugging of this setup,
  not by `ckpt intermittent` itself.
...
| Otii `GPI1` (expansion port) | MSP430 `P3.4` | Start/stop pulses (see below); shared with the Saleae channel |
...
| Saleae digital channel 0 | MSP430 `P3.4` | Always connected; verification/debugging only |
...
1. **Flash** — relays closed (`GPO2` high), Otii main off. The ELF is
   programmed via `mspdebug` and the target is held halted under JTAG.
...
3. **Replay** — the Otii main output replays the trace
   (`benchmarks/traces/*.csv`, 20 ms samples) while GPI1 is polled for the
   BENCH_EXIT stop pulse (~5 ms high). Replay stops early on the pulse, or at
   the end of the trace (`status=incomplete`).
...
2. Install the Python bindings: `uv sync --extra otii`.
3. Run, e.g.: `uv run ckpt intermittent milp crc --trace 1,2` (capacitor
